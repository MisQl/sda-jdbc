...
public class Main9Transaction {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        try {
            statement.executeUpdate("UPDATE user SET wrongcolumn='Alina' WHERE id=3");      // to zapytanie jest niepoprawne i rzuca wyjątek
        } catch (SQLException e) {
            connection.rollback(savepoint);
        }
        statement.executeUpdate("UPDATE user SET username='Bożydar' WHERE id=4");           // ta zmiana się wykona
        statement.close();
